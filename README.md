...
[`example/`](packages/drafter/example). Run it on any platform:
...
For markets-grade charts, this repo ships a sibling package,
[**`drafter_finance`**](packages/drafter_finance) — native candlestick / K-line,
OHLC, line, area, baseline, histogram and volume charts, with technical
indicators (SMA / EMA / RSI / MACD / Bollinger) and a magnet crosshair. No
WebView. It's built on a pure-Dart engine
([**`drafter_finance_engine`**](packages/drafter_finance_engine)) shared with the
Compose and SwiftUI ports, so all three render identically.

```yaml
dependencies:
  drafter_finance: ^0.1.0
```
